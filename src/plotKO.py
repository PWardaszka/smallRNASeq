...
if 'Tent2 KO' in df.columns and 'category' in df.columns:
    # Usuń wiersze, gdzie brakuje wartości w KO lub category
    df = df.dropna(subset=['Tent2 KO', 'category'])

    # Zsumuj liczbę zliczeń dla każdej kategorii
    category_sums = df.groupby('category')['Tent2 KO'].sum()
    ...
    category_sums.plot.pie(
        ...
        figsize=(6, 6),
        ylabel='',
        title='Udział zliczeń miRNA (kolumna Tent2 KO) wg kategorii'
    )
    ...
    print("Brakuje kolumn 'Tent2 KO' lub 'category' w pliku CSV.")
